...
app.post('/api/tregpost', function(req, res, next) {
  ...
  app.post('/treg', function(req, res, next) {
      ...
      var sql = `INSERT INTO readers( id, title, subject, Author, publication, email)  VALUES ("${tid}","${tname}","${depart}","teacher","${mno}","${email}" )`;
      mysqlConnection.query(sql, function(err, result) {
        if(err) {
  ...
  app.post('/sreg', function(req, res, next) {
      ...
      var sql = `INSERT INTO readers( id, title, subject, Author, publication, email)  VALUES ("${tid}","${tname}","${depart}","student","${mno}","${email}" )`;
      mysqlConnection.query(sql, function(err, result) {
        if(err) {
          res.status(500).send({ error: 'Something failed!' })
        }
